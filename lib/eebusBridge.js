...
class EEBusBridge extends EventEmitter {
	...
	waitForConnection() {
		return new Promise((resolve, reject) => {
			...
			// Listen for first successful message exchange
			const checkConnection = () => {
				clearTimeout(timeout);
				this.removeListener('event', checkConnection);
				resolve();
			};

			// For now, assume connection is ready once process spawns
			// TODO: Implement proper handshake with Go process
			setTimeout(() => {
				clearTimeout(timeout);
				resolve();
			}, 1000);
	...
	async stop() {
		...
		return new Promise((resolve) => {
			...
			if (this.process) {
				...
				setTimeout(() => {
					if (this.process) {
						this.adapter.log.warn('Force killing bridge process');
