...
class EEBusBridge extends EventEmitter {
	...
	detectBinaryPath() {
		...
		// Binary name: eebus-bridge-<goos>-<goarch>[.exe]
		let binaryName = `eebus-bridge-${goos}-${goarch}`;
		if (platform === 'win32') {
			binaryName += '.exe';
		}

		// Construct path: bin/eebus-bridge-<goos>-<goarch>[.exe]
		const binaryPath = path.join(__dirname, '..', 'bin', binaryName);

		this.adapter.log.debug(`Detected binary path: ${binaryPath} (platform: ${platform}/${goos}, arch: ${arch}/${goarch})`);
		return binaryPath;
	}
