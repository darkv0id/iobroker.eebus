...
const { validateSKI } = require('./skiValidator');
...
class StateManager {
	...
	async createDevice(device) {
		validateSKI(device.ski);
		const deviceId = device.ski;
	...
	/**
	 * Ensure device exists, create with minimal info if not
	 * @param {string} ski - Device SKI
	 * @returns {Promise<void>}
	 */
	async ensureDeviceExists(ski) {
		validateSKI(ski);
		const deviceId = ski;
		...
		this.adapter.log.info(`Creating device structure for newly connected device ${deviceId}`);
